...
public class RegionRenderer {
  ...
  protected void preRender(RegionFile rf, int[] colors, short[] heights) {
    new PreRenderer(rf, colors, heights).preRender();
  }
  ...
  private class PreRenderer {
    private RegionFile rf;
    private int[] colors;
    private short[] heights;
    private final int maxSectionCount = 16;
    private final short[][] sectionBlockIds = new short[maxSectionCount][16 * 16 * 16];
    private final byte[][] sectionBlockData = new byte[maxSectionCount][16 * 16 * 16];
    private final boolean[] usedSections = new boolean[maxSectionCount];
    private final byte[] biomeIds = new byte[16 * 16];

    public PreRenderer(RegionFile rf, int[] colors, short... heights) {
      this.rf = rf;
      this.colors = colors;
    ...
    public void preRender() {
      for (int cz = 0; cz < 32; ++cz) {
        for (int cx = 0; cx < 32; ++cx) {
          preRenderChunk(cz, cx);
        }
      }
    }

    private void preRenderChunk(int cz, int cx) {
      resetInterval();
      ...
      try {
        ...
        for (int z = 0; z < 16; ++z) {
          for (int x = 0; x < 16; ++x) {
            new ColumnPreRenderer(cz, cx, z, x).preRender();
          }
        }
    ...
    private class ColumnPreRenderer {
      private int cz;
      private int cx;
      private int z;
      private int x;
      private int pixelColor = 0;
      private short pixelHeight = 0;
      private boolean diamond = false;
      private boolean air = false;
      private boolean buriedAir = false;
      private boolean torch = false;
      private int biomeId;

      public ColumnPreRenderer(int cz, int cx, int z, int x) {
        this.cz = cz;
        this.cx = cx;
        this.z = z;
        this.x = x;
      ...
      private void renderSection(int s) {
        if (usedSections[s]) {
          ...
          for (int idx = z * 16 + x, y = 0, absY = s * 16; y < 16; ++y, idx += 256, ++absY) {
            int blockId = blockIds[idx] & 0xFFFF;
            byte blockDatum = blockData[idx];
            int blockColor = getBlockColor(blockId, blockDatum);
      ...
      private int getBlockColor(int blockId, byte blockDatum) {
        if (rendererCommand.showDiamonds && blockId == 56) // diamond
          diamond = true;
        if (air && (blockId == 1)) // stone over air
          buriedAir = true;
        if (rendererCommand.showAir && blockId == 0) // air
          air = true;
        if (rendererCommand.showTorches && blockId == 50) { // torch
          torch = true;
        }
        int blockColor = getColor(blockId, blockDatum, biomeId);
